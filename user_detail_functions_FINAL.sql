-- ============================================================================
-- USER DETAIL PAGE - DATABASE FUNCTIONS (FINAL VERSION)
-- ============================================================================
-- Updated to use actual column names and leverage new tables:
-- - user_achievements
-- - user_statistics
-- - user_weekly_percentiles
-- - scheduled_messages
-- Enhanced mistake analysis and vocabulary insights
-- Run this entire file in Supabase SQL Editor
-- ============================================================================

-- Function 1: Get comprehensive user statistics (FINAL - with correct column names)
...
BEGIN
    ...
    SELECT 
        ...
        -- Success rate from user_progress: times_correct / times_seen
        COALESCE((SELECT ROUND(
...
-- Function 3: Get user weekly performance (rankings & percentiles)
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(v.word, 'Unknown') as word,
        sm.status::TEXT,
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(up.difficulty_level, 'Unknown') as difficulty_level,
        STRING_AGG(DISTINCT um.mistake_type, ', ') as mistake_types,
        STRING_AGG(DISTINCT um.severity, ', ') as severity_distribution,
        v.chapter,
        v.type as word_type
...
BEGIN
    ...
    WITH mistake_totals AS (
        SELECT 
            COALESCE(um.mistake_category, 'Uncategorized') as category,
            COUNT(*) as count,
            MODE() WITHIN GROUP (ORDER BY um.mistake_type) as common_type
        FROM user_mistakes um
        WHERE um.user_id = p_user_id
...
BEGIN
    ...
    SELECT 
        ...
        -- Count correct from user_progress updates on that day
...
BEGIN
    ...
    SELECT 
        ...
        ls.created_at,
        COALESCE(v.word, 'Unknown') as word,
        COALESCE(v.translation, '') as translation,
        COALESCE(ls.session_type, 'learning') as session_type,
        (ls.user_response IS NOT NULL) as had_response,
        ls.user_response as response_value,
...
-- Function 11: Get user's learning patterns (time of day analysis)
...
-- ============================================================================
-- QUICK TEST - Uncomment to test each function
-- ============================================================================
...
-- SELECT * FROM get_user_upcoming_messages('YOUR_USER_ID', 5);
-- SELECT * FROM get_user_challenging_words('YOUR_USER_ID', 10);
-- SELECT * FROM get_user_mistake_analysis('YOUR_USER_ID');
-- SELECT * FROM get_user_progress_timeline('YOUR_USER_ID', 30);
-- SELECT * FROM get_user_recent_activity('YOUR_USER_ID', 20);
-- SELECT * FROM get_user_word_mastery('YOUR_USER_ID');
-- SELECT * FROM get_user_vocabulary_by_chapter('YOUR_USER_ID');
-- SELECT * FROM get_user_learning_patterns('YOUR_USER_ID');
-- SELECT * FROM get_user_insights('YOUR_USER_ID');

-- ============================================================================
-- DONE! 12 comprehensive functions ready for enhanced dashboard
-- ============================================================================
