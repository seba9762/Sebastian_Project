-- ============================================================================
-- USER DETAIL PAGE - DATABASE FUNCTIONS (UPDATED FOR YOUR SCHEMA)
-- ============================================================================
-- Updated to use 'vocabulary' table and your actual relationships
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE((SELECT ROUND(
            100.0 * COUNT(*) / 
            NULLIF((SELECT COUNT(*) FROM learning_sessions WHERE user_id = u.id), 0), 1
        ) FROM user_responses WHERE user_id = u.id AND is_correct = true), 0)::NUMERIC as success_rate,
...
BEGIN
    ...
    SELECT 
        ...
        EXISTS(SELECT 1 FROM user_responses WHERE session_id = ls.id) as had_response,
        EXISTS(SELECT 1 FROM user_responses WHERE session_id = ls.id AND is_correct = true) as was_correct,
        EXISTS(SELECT 1 FROM user_mistakes WHERE session_id = ls.id) as had_mistake
    FROM learning_sessions ls
